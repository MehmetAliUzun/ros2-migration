...
namespace Eigen
{
    ...
    template <>
    inline void to_json<Vector3f>(nlohmann::json& j, const MatrixBase<Vector3f>& vector)
    {
        j["x"] = vector.x();
        j["y"] = vector.y();
        j["z"] = vector.z();
    ...
    template <>
    inline void from_json<Vector3f>(const nlohmann::json& j, MatrixBase<Vector3f>& vector)
    {
